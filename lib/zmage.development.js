...
var renderContainer, renderPortal;
...
Zmage_ReactZmage.browsing = function (_ref) {
  var browsing = _ref.browsing,
      ...
      hotKey = _ref.hotKey,
      restProps = _objectWithoutProperties(_ref, ["browsing", "set", "src", "alt", "txt", "controller", "hotKey"]);

  // Init env
  renderContainer = document.body;
  renderPortal = document.createElement('div');
  renderPortal.id = 'zmagePortal';
  renderContainer.appendChild(renderPortal); // Mount target

  external_react_dom_default.a.render(external_react_default.a.createElement(Browser_Browser_Browser // Controlled status
  , _extends({
    browsing: true // Internal
    ,
    onBrowsing: function onBrowsing(status) {
      return !status && renderContainer.removeChild(renderPortal);
    } // Data
    ,
    set: convertSet({
      set: set,
    ...
    ,
    controller: Zmage_objectSpread({}, default_defProp.controller, controller),
    hotKey: Zmage_objectSpread({}, default_defProp.hotKey, hotKey)
  }, restProps)), renderPortal);
