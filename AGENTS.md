...
import type {
  ...
  ControllerSet, HotKey, Animate, AnimateFlip, AnimateCoverOptions,
...
2. **Hard-coding `preset='desktop'` on a touch-targeted page** — the desktop bundle ships hotkeys + arrow buttons, enables wheel zoom while zoomed, and disables mobile `gesture.swipe` / `gesture.dragExit`. Use `'auto'` (CSS-media-query-driven) or `'mobile'` explicitly when serving touch users.
