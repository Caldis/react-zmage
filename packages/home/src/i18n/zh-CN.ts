export const zhCN = {
  ...
  'useCases.title': '适合博客、CMS、MDX 和新闻页面的 React 图片预览',
  'useCases.body': 'react-zmage 是一个 React 图片查看器, 可以把普通 <img> 变成可原位展开的全屏图片预览。它支持多图浏览、键盘操作、移动端手势、事件触发、富文本 Wrapper 模式, 也支持 SSR/RSC 场景。',
  'useCases.cta.docs': '查看文档',
  'useCases.cta.wrapper': '试试 Wrapper 模式',
  'useCases.card.blog.title': '博客图片预览',
  'useCases.card.blog.body': '让文章里的图片可以全屏查看, 不需要改掉原本的文章排版, 也不需要额外做一套相册页面。',
  'useCases.card.cms.title': 'CMS 富文本图片',
  'useCases.card.cms.body': '包住 CMS、Markdown 解析器或编辑器输出的 HTML, 让已有 <img> 自动接入查看器。',
  'useCases.card.news.title': '新闻文章图集',
  'useCases.card.news.body': '正文仍然保持可读, 读者需要看细节时再进入支持键盘、手势和多图浏览的查看器。',
  'useCases.card.mdx.title': 'MDX 和文档图片',
  'useCases.card.mdx.body': '适合内容作者负责图片、React 应用负责页面外壳的文档页面, 直接用 Wrapper 模式接入。',
  'useCases.card.lightbox.title': 'React lightbox 替代方案',
  'useCases.card.lightbox.body': '当你只需要一层原位展开的图片预览, 又想保留现有 HTML 结构时, 不必接入更重的相册框架。',
  'useCases.card.ssr.title': 'Next.js、SSR 和 RSC',
  'useCases.card.ssr.body': '服务端渲染应用可以使用 SSR 安全入口, 再在客户端边界里打开交互式查看器。',
  'useCases.mode.eyebrow': '如何选择模式',
  'useCases.mode.title': '按你是否掌控图片 HTML 来选',
  'useCases.mode.component': '你掌控 JSX, 可以直接把 <img> 换成 <Zmage>。',
  'useCases.mode.imperative': '按钮、命令、事件或回调需要主动打开查看器。',
  'useCases.mode.wrapper': '图片来自 CMS、MDX、Markdown 或富文本 HTML。',
  'useCases.facts.eyebrow': '实体信息',
  'useCases.facts.title': '希望搜索引擎和 AI 抓住的简短事实',
  'useCases.fact.react': '兼容 React 16.8 到 19',
  'useCases.fact.modes': '支持组件、命令式和 Wrapper 三种模式',
  'useCases.fact.input': '支持键盘导航和移动端手势',
  'useCases.fact.ssr': '提供 SSR/RSC 安全入口',
  'useCases.fact.gallery': '不重构页面也能做多图浏览',
  'useCases.fact.origin': '原位展开的全屏图片预览',
  'useCases.faq.eyebrow': 'FAQ',
  'useCases.faq.title': '人和 AI Agent 通常会问的问题',
  'useCases.faq.what.q': 'react-zmage 是什么?',
  'useCases.faq.what.a': 'react-zmage 是一个 React 图片查看器, 可以把普通 <img> 变成可原位展开的全屏图片预览, 并支持多图浏览、键盘导航、移动端手势、Wrapper 模式和 SSR/RSC。',
  'useCases.faq.lightbox.q': '什么时候应该用 react-zmage, 而不是 React lightbox?',
  'useCases.faq.lightbox.a': '当你希望已有图片可以全屏查看, 但不想围绕相册组件重写页面结构时, react-zmage 更合适。博客、CMS 富文本、MDX 文档、新闻文章和图片较多的内容页都适合。',
  'useCases.faq.richText.q': 'react-zmage 能处理 CMS、Markdown 或富文本图片吗?',
  'useCases.faq.richText.a': '可以。Zmage.Wrapper 会绑定后代 <img>, 适合 CMS、Markdown、MDX 或 dangerouslySetInnerHTML 渲染出来的内容, 同时保留原页面结构。',
  'useCases.faq.ssr.q': 'react-zmage 支持 Next.js、SSR 或 RSC 吗?',
  'useCases.faq.ssr.a': '支持。SSR 或 RSC 场景使用 react-zmage/ssr 子路径导入, 交互式查看器 API 放在客户端事件或 Client Component 中调用。',
  ...
  'docs.search.desc.propsInterface': 'backdrop · radius · edge · loop · zIndex · portalTarget 等界面参数',
  ...
  'docs.section.props.wrapperScope.config': 'set 和 defaultPage 可用于显式共享图库。若被点击图片的 src 出现在 set 中,Wrapper 会打开匹配索引;defaultPage 只作为兜底。preset、controller、hotKey、animate、backdrop、zIndex、portalTarget、radius、edge、loop、coverVisible、hideOnScroll、hideOnDblClick、loadingDelay 正常生效。',
  ...
  'docs.section.props.portalTarget.title': '自定义 Portal 挂载点',
  'docs.section.props.portalTarget.body': '默认情况下,浏览层挂到 document.body。若你的应用已经有统一的 overlay root、modal root、shadow host 或微前端 shell 容器,可以把该元素传给 portalTarget,方便统一管理层级、样式作用域和宿主框架的生命周期。',
  'docs.section.props.portalTarget.boundary': 'portalTarget 只改变 DOM 父节点。查看器仍使用 fixed 全屏布局,不会被挂载容器裁剪成局部预览;需要调整遮罩层级时继续使用 zIndex。',
  ...
  'docs.section.examples.portalTargetTitle': '挂到宿主 overlay root',
  'docs.section.examples.portalTargetBody': '当页面有统一的弹层容器时,把该 DOM 节点传给 portalTarget。示例仍是全屏查看器,只是 Portal 的父节点换成了宿主 root。',
