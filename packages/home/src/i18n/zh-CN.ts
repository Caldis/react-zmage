export const zhCN = {
  ...
  'controller.layout.desc': '调整工具栏、分页器和 caption 的覆盖层位置。数值按 px 处理,字符串可传任意 CSS 长度;layout.mobile 可覆盖移动端位置。',
  'controller.layout.paginationBottom.desc': '分页器距离底部的偏移量,用于避开 caption 或页面自定义 UI。',
  'controller.layout.captionBottom.desc': 'caption 距离底部的偏移量,用于避免与分页器或底部控制区相切。',
  ...
  'docs.section.props.controller.layoutIntro': 'layout 只调整控制器、分页器和 caption 的覆盖层偏移,不参与图片首帧和缩放动画的几何计算。数值按 px 处理,字符串按 CSS 长度透传;layout.mobile 会在移动端覆盖基础布局。',
