...
export const ko: I18nDict = {
  ...
  'hero.cta.playground': '원위치 펼치기 체험',
  ...
  'controller.layout.desc': '툴바, 페이지네이션, caption 오버레이 위치를 조정합니다. 숫자는 px, 문자열은 CSS 길이로 처리되며 layout.mobile은 모바일 preset에서 덮어씁니다.',
  'controller.layout.paginationBottom.desc': '페이지네이션의 하단 inset입니다. caption이나 사용자 UI와 겹칠 때 여백을 만들 수 있습니다.',
  'controller.layout.captionBottom.desc': 'caption의 하단 inset입니다. 페이지네이션이나 하단 컨트롤보다 위에 두고 싶을 때 사용합니다.',
  ...
  'docs.section.props.controller.layoutIntro': 'layout은 controller, pagination, caption 오버레이 offset만 조정하며 이미지 첫 프레임이나 zoom 지오메트리에는 참여하지 않습니다. 숫자는 px, 문자열은 CSS 길이로 전달되고 layout.mobile은 모바일에서 기본 레이아웃을 덮어씁니다.',
