...
export const ja: I18nDict = {
  ...
  'controller.layout.desc': 'ツールバー、ページネーション、caption のオーバーレイ位置を調整します。数値は px、文字列は CSS 長として扱われ、layout.mobile はモバイル preset で上書きされます。',
  'controller.layout.paginationBottom.desc': 'ページネーションの下端 inset。caption やページ側 UI と重なる場合に余白を作れます。',
  'controller.layout.captionBottom.desc': 'caption の下端 inset。ページネーションや下部コントロールより上に置きたい場合に使います。',
  ...
  'docs.section.props.controller.layoutIntro': 'layout は controller、pagination、caption のオーバーレイ offset だけを調整します。画像の初期フレームやズームジオメトリには関与しません。数値は px、文字列は CSS 長として透過され、layout.mobile はモバイルで基礎配置を上書きします。',
