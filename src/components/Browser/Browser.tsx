...
export interface PropsType extends PresetParams, FunctionalParams, InterfaceAndInteractionParams, LifeCycleParams {
    // Controlled status
...
export default class Browser extends React.Component<PropsType, StateType> {
     ...
     */
    init = () => {
        const { isBrowsingControlled, coverRef, onBrowsing, hideOnScroll, coverVisible, presetIsDesktop } = this.getPropsWithEnv()
    ...
    unInit = ({ force }={ force: false }) => {
        const { isBrowsingControlled, coverRef, onBrowsing, hideOnScroll, coverVisible, presetIsMobile, presetIsDesktop } = this.getPropsWithEnv()
        const { show, pageIsCover } = this.state
        if (show || force) {
