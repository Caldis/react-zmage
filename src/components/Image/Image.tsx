...
import { Context, BrowsingParams, ContextType } from '../context'
import {
...
interface PropsType extends BrowsingParams {}
interface StateType {
    // 加载状态
    isFetching: boolean
    invalidate: boolean
...
export default class Image extends React.Component<PropsType, StateType> {
    ...
    getStyle = (step: number, distance: number, isSideImage: boolean): CSSProperties => {
        ...
        // 获取触摸配置
        let { touch, transition } = touchProfile.getTouchConfig({ enableSwiping:set.length>1, enableLiving:true })
        // 计算样式
        if (isSideImage) {
            // 仅对左右两张图做滑动跟踪
