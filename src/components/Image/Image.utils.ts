...
// Utils
import { calcFitScale, getScrollWidth, getInnerHeight, numberOfStyleUnits } from '@/utils'
import { animationTransition } from '@/config/anim.ts'
...
/* 获取缩放样式 */
export const getZoomingStyle = (context: ContextType, imageRef: RefObject<HTMLImageElement>, { clientX:mouseX=getScrollWidth()/2, clientY:mouseY=getInnerHeight()/2 }={}): ImageStyleType => {
    const { radius, edge, rotate } = context
    const { naturalWidth, naturalHeight } = imageRef.current
    // 随鼠标位移偏移量
